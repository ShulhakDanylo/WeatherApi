...
const EmailIcon = () => (
    <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
        <rect x="1.5" y="4" width="15" height="11" rx="2" stroke="rgba(255,255,255,0.50)" strokeWidth="1.4"/>
        <path d="M1.5 7l7.5 5 7.5-5" stroke="rgba(255,255,255,0.50)" strokeWidth="1.4" strokeLinecap="round"/>
    </svg>
);
...
export default function RegisterForm({ onClose, onRegister, onSwitchToLogin }) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const data = await authService.register(nickname, email);
            localStorage.setItem('user', JSON.stringify(data)); // Зберігаємо результат
            onRegister && onRegister(data);
            onClose();
        } catch (err) {
            console.error("Помилка реєстрації:", err);
            alert("Не вдалося зареєструватися. Перевір чи запущений бекенд.");
        }
    };
    ...
    return (
        <div className="auth-overlay" onClick={onClose}>
            <div className="auth-card" onClick={e => e.stopPropagation()}>
                ...
                <p className="auth-sub">Створіть свій безкоштовний акаунт за лічені секунди</p>
                ...
                <button className="auth-btn-primary" onClick={handleSubmit} disabled={!nickname.trim()}>
                    Створити акаунт <span className="auth-arrow">→</span>
                </button>

                <p className="auth-switch-hint">
                    Вже маєте акаунт?{' '}
                    <button className="auth-link" onClick={onSwitchToLogin}>Увійти</button>
                </p>
            </div>
        </div>
    );
